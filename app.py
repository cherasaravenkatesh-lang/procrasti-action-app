...
# --- Pomodoro and Helper Functions (No changes) ---
...
# --- Dashboard & Calendar Tabs (No changes) ---
with tab1:
...
# --- All Tasks Tab (CORRECTED STRUCTURE) ---
with tab2:
    st.header("All Tasks")
    with st.expander("Add a New Task"):
        with st.form("new_task_form", clear_on_submit=True):
            ...
            questions = st.text_area("General Questions for this task"); subtasks = st.text_area("Sub-tasks (one per line)")
            st.markdown("**Link People & Questions**"); linked_people_names = st.multiselect("Select people to link to this task", options=people_names)
    ...
    for task in tasks:
        if task['status'] in filter_status:
            ...
            with st.status(f"{task['title']}{due_date_str}", expanded=False, state=("complete" if task['status'] == 'Completed' else "running")):
                ...
                # === UI Management Section (OUTSIDE THE FORM) ===
                ...
                for i, person in enumerate(st.session_state[session_key]):
                    ...
                    # THIS IS THE BUTTON CAUSING THE ERROR, NOW SAFELY OUTSIDE THE FORM
                ...
                # === Data Submission Section (INSIDE THE FORM) ===
                with st.form(key=f"form_{task['id']}"):
                    ...
                    if btn_cols[0].form_submit_button("Save Changes", type="primary", use_container_width=True):
                        ...
                        del st.session_state[session_key]; st.toast(f"Updated '{new_title}'", icon="💾"); st.rerun()
        ...
        with st.form("new_person_form", clear_on_submit=True):
            ...
            if st.form_submit_button("Add Person", type="primary"):
                if person_name:
                    ...
                    except sqlite3.IntegrityError: st.error(f"'{person_name}' already exists in your contacts.")
                    except Exception as e: st.error(f"An error occurred: {e}")
                else: st.warning("Please enter a name.")
    if not people:
        st.info("You haven't added any people yet.")
    else:
